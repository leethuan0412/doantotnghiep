...
export default function ProductStateScreen({route}) {
  ...
  function SendNotificationToUser(playerIds, statusOrder) {
    ...
    const data = {
      ...
      type: 'public',
      headings: {
        en: 'Bạn có một thông báo từ ThuanMart',
      },
      contents: {
      ...
      <View style={{flex: 1, position: 'relative'}}>
        <View style={{flex: 1, alignItems: 'center'}}>
          ...
          <Picker
            ...
            <Picker.Item label={Orders.status} value="xác nhận" />
            <Picker.Item
              label="chờ lấy hàng"
              value="chờ lấy hàng"
            />
            <Picker.Item label="Đang giao hàng" value="Đang giao hàng" />
            <Picker.Item label="Giao thành công" value="Giao thành công" />
            <Picker.Item label="hủy đơn" value="hủy đơn" />
          </Picker>
        </View>
        <TouchableOpacity
          activeOpacity={0.8}
          onPress={() =>
            firestore()
              ...
              .then(() => {
                if (selectedValue !== 'Giao thành công') {
                  if (selectedValue == 'hủy đơn') {
                    SendNotificationToUser(
                      playerIds,
                      selectedValue === 'hủy đơn'
                        ? 'Đơn hàng đã bị hủy'
                        : '',
                    );
